...
class DraftConsumer(MessageConsumer):
    ...
    def receive_json(self, content, **kwargs):
        ...
        if message_type == 'join':
            if not isinstance(content['draft'], str):
                self._send_error('invalid draft id field')
                # self._send_message('join', state = 'failure', reason = 'invalid draft id field')
                return

            try:
                draft_id = uuid.UUID(content['draft'])
            except ValueError:
                self._send_error('invalid draft id field')
                return

            draft = DRAFT_COORDINATOR.get_draft_slot(draft_id)
            if draft is None:
